...
@Controller('api/groupstudies')
export class GroupstudiesController {
  // สร้าง instance ของ PrismaService เพื่อใช้ติดต่อ DB
  constructor() { 
    this.prisma = new PrismaService(); 
  }

  // กำหนด endpoint: GET /groupstudies
  @Get()
  async getGroupstudies() {
    ...
    const res = await fetch('https://triup.tsri.or.th/service/api/groupstudies', {
    ...
    if (!res.ok) {
      ...
      return { 
        success: false, 
        error: `groupstudies fetch failed: ${res.status} ${res.statusText} ${text}` 
      };
    }
