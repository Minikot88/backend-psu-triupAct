...
@Controller('api/form_utilization')
export class Form_utilizationController {
  // สร้าง instance ของ PrismaService เพื่อใช้ติดต่อ DB
  constructor() { 
    this.prisma = new PrismaService(); 
  }

  // กำหนด endpoint: GET /form_utilization
  @Get()
  async getForm_utilization() {
    ...
    const res = await fetch('https://triup.tsri.or.th/service/api/form_utilization', {
    ...
    if (!res.ok) {
      ...
      return { 
        success: false, 
        error: `form_utilization fetch failed: ${res.status} ${res.statusText} ${text}` 
      };
    }
