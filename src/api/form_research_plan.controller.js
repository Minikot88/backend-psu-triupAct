...
@Controller('api/form_research_plan')
export class Form_research_planController {
  // สร้าง instance ของ PrismaService เพื่อใช้ติดต่อ DB
  constructor() { 
    this.prisma = new PrismaService(); 
  }

  // กำหนด endpoint: GET /form_research_plan
  @Get()
  async getForm_research_plan() {
    ...
    const res = await fetch('https://triup.tsri.or.th/service/api/form_research_plan', {
    ...
    if (!res.ok) {
      ...
      return { 
        success: false, 
        error: `form_research_plan fetch failed: ${res.status} ${res.statusText} ${text}` 
      };
    }
