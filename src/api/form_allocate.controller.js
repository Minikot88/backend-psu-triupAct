...
@Controller('api/form_allocate')
export class Form_allocateController {
  // สร้าง instance ของ PrismaService เพื่อใช้ติดต่อ DB
  constructor() { 
    this.prisma = new PrismaService(); 
  }

  // กำหนด endpoint: GET /form_allocate
  @Get()
  async getForm_allocate() {
    ...
    const res = await fetch('https://triup.tsri.or.th/service/api/form_allocate', {
    ...
    if (!res.ok) {
      ...
      return { 
        success: false, 
        error: `form_allocate fetch failed: ${res.status} ${res.statusText} ${text}` 
      };
    }
