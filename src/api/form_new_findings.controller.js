...
@Controller('api/form_new_findings')
export class Form_new_findingsController {
  // สร้าง instance ของ PrismaService เพื่อใช้ติดต่อ DB
  constructor() { 
    this.prisma = new PrismaService(); 
  }

  // กำหนด endpoint: GET /form_new_findings
  @Get()
  async getForm_new_findings() {
    ...
    const res = await fetch('https://triup.tsri.or.th/service/api/form_new_findings', {
    ...
    if (!res.ok) {
      ...
      return { 
        success: false, 
        error: `form_new_findings fetch failed: ${res.status} ${res.statusText} ${text}` 
      };
    }
