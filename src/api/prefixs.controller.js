...
export class PrefixsController {
  ...
  async getPrefixs() {
    ...
    const res = await fetch('https://triup.tsri.or.th/service/api/prefixs', {
    ...
    if (!res.ok) {
      ...
      return { 
        success: false, 
        error: `prefixs fetch failed: ${res.status} ${res.statusText} ${text}` 
      };
    }
