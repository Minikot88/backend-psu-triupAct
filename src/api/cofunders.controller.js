...
@Controller('api/cofunders')
export class CofundersController {
  // สร้าง instance ของ PrismaService เพื่อใช้ติดต่อ DB
  constructor() { 
    this.prisma = new PrismaService(); 
  }

  // กำหนด endpoint: GET /cofunders
  @Get()
  async getCofunders() {
    ...
    const res = await fetch('https://triup.tsri.or.th/service/api/cofunders', {
    ...
    if (!res.ok) {
      ...
      return { 
        success: false, 
        error: `cofunders fetch failed: ${res.status} ${res.statusText} ${text}` 
      };
    }
