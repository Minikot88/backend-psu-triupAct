...
@Controller('api/target_audiences')
export class Target_audiencesController {
  // สร้าง instance ของ PrismaService เพื่อใช้ติดต่อ DB
  constructor() { 
    this.prisma = new PrismaService(); 
  }

  // กำหนด endpoint: GET /target_audiences
  @Get()
  async getTarget_audiences() {
    ...
    const res = await fetch('https://triup.tsri.or.th/service/api/target_audiences', {
    ...
    if (!res.ok) {
      ...
      return { 
        success: false, 
        error: `target_audiences fetch failed: ${res.status} ${res.statusText} ${text}` 
      };
    }
