...
@Controller('api/substudies')
export class SubstudiesController {
  // สร้าง instance ของ PrismaService เพื่อใช้ติดต่อ DB
  constructor() { 
    this.prisma = new PrismaService(); 
  }

  // กำหนด endpoint: GET /substudies
  @Get()
  async getSubstudies() {
    ...
    const res = await fetch('https://triup.tsri.or.th/service/api/substudies', {
    ...
    if (!res.ok) {
      ...
      return { 
        success: false, 
        error: `substudies fetch failed: ${res.status} ${res.statusText} ${text}` 
      };
    }
