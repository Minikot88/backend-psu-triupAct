...
@Controller('api/findingdetaillists')
export class FindingdetaillistsController {
  // สร้าง instance ของ PrismaService เพื่อใช้ติดต่อ DB
  constructor() { 
    this.prisma = new PrismaService(); 
  }

  // กำหนด endpoint: GET /findingdetaillists
  @Get()
  async getFindingdetaillists() {
    ...
    const res = await fetch('https://triup.tsri.or.th/service/api/findingdetaillists', {
    ...
    if (!res.ok) {
      ...
      return { 
        success: false, 
        error: `findingdetaillists fetch failed: ${res.status} ${res.statusText} ${text}` 
      };
    }
