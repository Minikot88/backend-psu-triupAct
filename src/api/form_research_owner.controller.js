...
@Controller('api/form_research_owner')
export class Form_research_ownerController {
  // สร้าง instance ของ PrismaService เพื่อใช้ติดต่อ DB
  constructor() { 
    this.prisma = new PrismaService(); 
  }

  // กำหนด endpoint: GET /form_research_owner
  @Get()
  async getForm_research_owner() {
    ...
    const res = await fetch('https://triup.tsri.or.th/service/api/form_research_owner', {
    ...
    if (!res.ok) {
      ...
      return { 
        success: false, 
        error: `form_research_owner fetch failed: ${res.status} ${res.statusText} ${text}` 
      };
    }
