...
@Controller('api/time_settings')
export class Time_settingsController {
  // สร้าง instance ของ PrismaService เพื่อใช้ติดต่อ DB
  constructor() { 
    this.prisma = new PrismaService(); 
  }

  // กำหนด endpoint: GET /time_settings
  @Get()
  async getTime_settings() {
    ...
    const res = await fetch('https://triup.tsri.or.th/service/api/time_settings', {
    ...
    if (!res.ok) {
      ...
      return { 
        success: false, 
        error: `time_settings fetch failed: ${res.status} ${res.statusText} ${text}` 
      };
    }
