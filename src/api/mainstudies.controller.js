...
@Controller('api/mainstudies')
export class MainstudiesController {
  // สร้าง instance ของ PrismaService เพื่อใช้ติดต่อ DB
  constructor() { 
    this.prisma = new PrismaService(); 
  }

  // กำหนด endpoint: GET /mainstudies
  @Get()
  async getMainstudies() {
    ...
    const res = await fetch('https://triup.tsri.or.th/service/api/mainstudies', {
    ...
    if (!res.ok) {
      ...
      return { 
        success: false, 
        error: `mainstudies fetch failed: ${res.status} ${res.statusText} ${text}` 
      };
    }
